...
#define GFACTOR (2.7725887222397811)                 // 4.0 * ln(2.0)


// gauss1d pars = { fwhm, pos, ampl }

template <typename DataType>
static inline void gauss1d(const DataType*& x, const int& num,
  ...
  for(int i = 0; i < num; i++) {
    ...
    res[i] = pars[2] * std::exp(-GFACTOR * diff * diff);
...
template <typename DataType>
static inline void gauss2d(const DataType*& x0, const DataType*& x1,
  ...
  for(int i = 0; i < num; i++) {
    ...
    res[i] = pars[5] * std::exp(-(radius/(pars[0]*pars[0]))*GFACTOR);
